...
export function render(ctx){
 ...
 root.innerHTML=(d.runs[0]&&d.runs[0].source_revision!==p.revision?'<div class="warning">当前排期已过期，日历展示的是上次保存的结果。</div>':'')+card('会议月历','<p class="muted">点击日期或会议查看负责人和班主任。切换月份会同步后台当前月份。</p><div id="calendar"></div>');
 ...
 initialView:'dayGridMonth',initialDate:`${p.year}-${String(p.month).padStart(2,'0')}-01`,locale:'zh-cn',firstDay:0,height:'auto',dayMaxEvents:3,
 ...
 events:booked.map(s=>({id:s.id,start:`${s.meeting_date}T${s.start_time}`,end:`${s.meeting_date}T${s.end_time}`,title:`${s.start_time.slice(0,5)} · ${d.assignments.filter(a=>a.slot_id===s.id).length}位班主任`,display:'block'})),
