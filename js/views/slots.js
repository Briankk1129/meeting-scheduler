...
export function render(ctx) {
 ...
 root.innerHTML=card('① 选择会议日期 · ② 设置当天时间段',`<form id="batch-slots"><p class="muted">先在日历选择一天或多天，再在日历下方填写开始时间和结束时间。可一次添加多个时间段，已有的相同时间会自动跳过。</p><div class="toolbar"><button type="button" class="light" data-days="all">全选本月</button><button type="button" class="light" data-days="weekdays">选择工作日</button><button type="button" class="light" data-days="none">清空选择</button><span id="date-count" class="badge">已选 0 天</span></div><div class="date-picker" aria-label="${p.year}年${p.month}月日期多选">${['一','二','三','四','五','六','日'].map(x=>`<span class="weekday">${x}</span>`).join('')}${'<span></span>'.repeat(offset)}${Array.from({length:days},(_,i)=>{const date=prefix+'-'+String(i+1).padStart(2,'0'),exists=d.slots.some(s=>s.meeting_date===date);return `<label class="date-cell"><input type="checkbox" name="dates" value="${date}" aria-label="${date}"><span>${i+1}${exists?'<small>已设置</small>':''}</span></label>`;}).join('')}</div><h3>② 这些日期可以开会的时间段</h3><div id="time-ranges" class="stack">${rangeRow()}</div><div class="toolbar"><button id="add-range" type="button" class="light">＋ 增加时间段</button></div><div class="form-grid">${input('capacity_override','每段人数上限（留空用月份默认）','number','','min="0" max="100"')}<button id="save-batch">添加所选日期的时间段</button></div><p class="footer-note">本月默认每段 ${p.default_capacity} 人；设为 0 可关闭该时间段的班主任名额。负责人不占人数。</p></form>`)+
