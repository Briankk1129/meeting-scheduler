...
export function render(ctx) {
 ...
 '<div id="meeting-times"></div><details class="card"><summary>查看全部月份</summary>'+card('月份列表',table(['月份','状态','默认容量','操作'],d.periods.map(x=>[h(x.title),h(statusText(x.status)),x.default_capacity,button('switch',x.id,'切换到此月')])) )+'</details>';
 bindForm('#period-create',async f=>{f.title=f.title.trim()||`${f.year}年${f.month}月会议`;await ctx.mutate('period_create',f);});
 bindForm('#period-edit',f=>ctx.mutate('period_update',f));bindActions((action,id)=>{if(action==='switch')return ctx.selectPeriod(id);});
 if(p)renderSlots({...ctx,root:root.querySelector('#meeting-times')});
}
